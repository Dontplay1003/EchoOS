...
extern void _set_tssldt_desc(unsigned short *n,unsigned long addr,char tp) 
{ 
	*n = 104;	// 描述符长度。
	*(n + 1) = addr;	// 段基地址。
	*(n + 2) = addr >> 16;	// 段基地址。
...
#define set_tss_desc(n,addr) _set_tssldt_desc((unsigned short*)(n),(unsigned long)(addr), (char)0x89)
//// 在全局表中设置局部表描述符。
// n - 是该描述符的指针；addr - 是描述符中的基地址值。局部表描述符的类型是0x82。
#define set_ldt_desc(n,addr) _set_tssldt_desc((unsigned short*)(n),(unsigned long)(addr), (char)0x82)
